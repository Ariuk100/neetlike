...
function sanitizeProfile(input: unknown): Record<string, any> {
  ...
  const phone = normPhone((input as any).phone)
  if (phone !== undefined) out.phone = phone

  const grade = normGrade((input as any).grade)
  if (grade !== undefined) out.grade = grade

  const by = normBirthYear((input as any).birthYear)
  if (by === 'DELETE') {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const newPhone: string | undefined = profileData.phone
      ...
      if (isNew) {
        ...
        await adminFirestore.runTransaction(async (tx) => {
          ...
          if (lock.exists && lock.data()?.uid !== uid) {
            const error = new Error('Phone already in use') as any
            error.status = 409
            throw error // ✅ Object.assign оронд энгийн error
          }
