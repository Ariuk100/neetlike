...
export function useCache(storageKeyPrefix: string = '', defaultOptions?: CacheOptions) {
  ...
  // ✅ Memory leak засах - cleanup effect нэмэх
  useEffect(() => {
    const listeners = changeListeners.current;
    return () => {
      // Component unmount үед бүх listeners цэвэрлэх
      listeners.clear();
    };
  }, []);
  ...
  const getAllKeys = useCallback(
    (storage: 'local' | 'session' = 'session'): string[] => {
      ...
      // Handle both real storage and mock storage
...
export function useCachedState<T>(key: string, defaultValue: T, options?: CacheOptions): [T, (data: T) => void] {
  ...
  useEffect(() => {
    const unsubscribe = cache.subscribe(key, (subscriptionKey, data) => {
      // Only update if this is the key we're watching
  ...
  const setCachedState = useCallback((data: T) => {
    ...
    setState(data); // Also update local state immediately
  }, [key, cache, options]);
